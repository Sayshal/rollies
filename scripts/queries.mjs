 ...
 */
async function handleRollRequest(queryData, _options) {
  ...
  const result = await showRollDialog(combatant, dieType, rolloffId, mode, opponents);
...
async function handleShowWinner(queryData, { timeout }) {
  ...
  if (showAnnouncement) {
    const timeoutPromise = new Promise((resolve) => {
      setTimeout(() => resolve({ acknowledged: true, timedOut: true }), timeout);
    });
